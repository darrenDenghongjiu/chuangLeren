...
EJS.Scanner = function(source, left, right) {
	...
	this.SplitRegexp = left=='[' ? /(\[%%)|(%%\])|(\[%=)|(\[%#)|(\[%)|(%\]\n)|(%\])|(\n)/ : new RegExp('('+this.double_left+')|(%%'+this.double_right+')|('+this.left_equal+')|('+this.left_html+')|('+this.left_comment+')|('+this.left_delimiter+')|('+this.right_delimiter+'\n)|('+this.right_delimiter+')|(\n)') ;
...
EJS.Scanner.to_html = function(input){
	if(input == null || input === undefined)
        return '';
    if(input instanceof Date)
		return input.toDateString();
				...
				case scanner.right_delimiter:
					switch(scanner.stag) {
						...
						case scanner.left_equal:
							...
							break;
						case scanner.left_html:
							buff.push(insert_cmd + "(EJS.Scanner.to_html(" + content + ")))");
							break;
					}
					scanner.stag = null;
					content = '';
...
EJS.Helpers.prototype = {
	...
	to_html: function(input, null_text){
		if(input == null || input === undefined) return null_text || '';
	    if(input instanceof Date) return input.toDateString();
		if(input.toString) return (''+input).split("<").join("&lt;").split(">").join("&gt;").split('"').join("&#34;").split("'").join("&#39;");;
		return '';
	}
};
